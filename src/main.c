...
int main(void) {
  ...
  if (aspect >= 1.0f) {
    glm_ortho(-viewSize * aspect / 2.0f, viewSize * aspect / 2.0f,
              -viewSize / 2.0f, viewSize / 2.0f, -1.0f, 1.0f, projectionMatrix);
  } else {
    glm_ortho(-viewSize / 2.0f, viewSize / 2.0f, -viewSize / aspect / 2.0f,
              viewSize / aspect / 2.0f, -1.0f, 1.0f, projectionMatrix);
  }
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    float r =
        sqrt(drawData[1].x * drawData[1].x + drawData[1].y * drawData[1].y);
